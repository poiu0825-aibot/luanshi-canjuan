...
(function(){
 ...
 const oldHero=SpriteSystem.hero.bind(SpriteSystem),oldPortrait=NPCSystem.render.bind(NPCSystem);
 ...
 function paint(canvas,unit,cls,id,now=performance.now()){
  if(!canvas)return;
  const p=profile(unit,cls),img=p&&imageFor(p.src);
  if(!art.enabled||!p||!img?.complete||!img.naturalWidth){oldHero(canvas,cls);return;}
  ...
  ctx.drawImage(img,frame*w,p.row*h,w,h,0,bob,size,size-bob);
 ...
 function redraw(now=performance.now()){
  for(const [id,a] of actors){if(!a.canvas.isConnected){actors.delete(id);continue;}paint(a.canvas,a.unit,a.cls,id,now);}
 }
 ...
 SpriteSystem.animate=function(attacker,index){if(attacker==='hero')strikes.set('hero',performance.now());animate(attacker,index);redraw();start();};
 ...
 QualitySystem.apply=function(redraw=true){applyQuality(redraw);const button=$('#qualityBtn');button.textContent=this.low?'畫質・簡化':'畫質・標準';button.title=this.low?'切換標準畫質':'切換簡化畫質';button.setAttribute('aria-label',this.low?'簡化畫質：角色 64px，敵人 64px':'標準畫質：角色 128px');};
 ...
 window.PixelArtSystem={profile,paint,settings:art,profiles,redraw};
